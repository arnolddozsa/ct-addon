...
/**
 * CityMedia szerződés lap.
 */
class Partner_Contract extends \UI\View\Pub\Pages\PageView{

	/**
	 *
	 * Szerződés konstruktora.
	 *
	 */

	public function __construct(){
		parent::__construct();
		$this->SetTitle(Trans::Get("partner_contract"));
	}
	...
	public function CreateContent(){
		...
		try {
			...
			$qe = new QE\Query();
			$t1 = $qe->AddFromTable("ct_partner_contract_item", "T1");
			...
			$field->SetReference($t1);
			$field = $qe->AddField("settlementByDifference");
			$field->SetReference($t1);
			$field = $qe->AddField("(SELECT SUM(quantity) FROM nubes_item_quantity WHERE itemId = T1.itemId)", "quantity");
			...
			$data["partnerContractItem"] = $dt_creator->FromDataSource($qe, $params);
			$data["partnerContractItem"]->SetRows(null);


			$data["partnerContractItem"]->GetColumn("id")->SetEditable(false);
			$data["partnerContractItem"]->GetColumn("id")->SetVisible(false);
