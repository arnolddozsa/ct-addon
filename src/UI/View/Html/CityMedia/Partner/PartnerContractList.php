 ...
 */
class PartnerContractList implements \UI\View\IView{
	...
	public function CreateContent(){
		...
		$input->SetValidValues(Field_Valid_Values::GetValidValuesFromDataBaseToSelect("nubes", "ct_partner_contract", "status"));
		$input->visible = true;
		$input->disabled = true;
		...
		$input = new \UI\Html\Select();
		$input->SetValidValues(Field_Valid_Values::GetValidValuesFromDataBaseToSelect("nubes", "ct_partner_contract", "type"));
		$input->visible = true;
		$input->disabled = true;
            ...
            $(function () {
                ...
                $(`${window.elementID}_datatable_list`).on("pageLoad", function(){
                    ...
                    $(this).find("tbody tr td:last-child() a").each(function(){
                        $(this).prop("href", `/CityMedia/Partner/Partner_Contract/?id=${($(this).find("button").val())}`);
                ...
                $(`${window.elementID}_addNew_button`).on("click", function () {
                    window.location = window.location.origin + "/CityMedia/Partner/Partner_Contract/";
                });
		...
		<h1 class="center-text"><?php echo Trans::Get("list_of_contracts"); ?></h1>
