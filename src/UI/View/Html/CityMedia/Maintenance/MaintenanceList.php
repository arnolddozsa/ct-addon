...
/**
 * Karbantartási bizonylat lista nézet osztály.
 */
class MaintenanceList implements \UI\View\IView{
	...
	public function CreateContent(){
            ...
            $(function () {
                ...
                $(`${window.elementID}_datatable_list`).on("pageLoad", function(){
                    ...
                    $(this).find("tbody tr td:last-child() a").each(function(){
                        $(this).prop("href", `/CityMedia/Maintenance/Maintenance/?id=${($(this).find("button").val())}`);
                ...
                $(`${window.elementID}_addNew_button`).on("click", function () {
                    window.location = window.location.origin + "/CityMedia/Maintenance/Maintenance/";
                });
		...
		<h1 class="center-text"><?php echo Trans::Get("list_of_maintenances"); ?></h1>
